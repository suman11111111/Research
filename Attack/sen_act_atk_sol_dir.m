...
%     if i==1000 
%         x1(i)=x1(i)+a*exp(i*dt);
%     end
    
    
    x1(i+1)=x1(i)+dt*(-k*(x1(i)-w1(i))+x_act);
    x2(i+1)=x2(i)+dt*((-1)*(x2(i)-x1(i))-k*(x2(i)-w2(i)));
    x3(i+1)=x3(i)+dt*((-1)*(x3(i)-x1(i))-k*(x3(i)-w3(i)));
    
    
    w1(i+1)=w1(i);
    w2(i+1)=w2(i)+dt*((-1)*(w2(i)-w1(i)));
    w3(i+1)=w3(i)+dt*((-1)*(w3(i)-w1(i)));
