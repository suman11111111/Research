...
[t,x]=ode113(@(t,x) sys(A,B,K,x,t,ta_i,ta_f),tspan,x0,options);
...
zoomY = [min(u_val(indexrange)), max(u_val(indexrange))];
rectangle('Position', [zoomX(1), zoomY(1), diff(zoomX), diff(zoomY)], ...
    'EdgeColor', 'r', 'LineWidth', 3);
insetAx = axes('Position', [0.6, 0.28, 0.28, 0.32]);
box on;
annotation('rectangle', insetAx.Position, 'Color', 'k', 'LineWidth', 2.5);
plot(insetAx, u_t, u_val, 'b-', 'LineWidth', 1.5);
...
xlim(zoomX)
ylim(zoomY)
legendObj2 = legend({'u^a'}, 'FontSize', 28, 'LineWidth', 1.5);
legendBox2 = findobj(legendObj2, 'Type', 'Patch');
set(legendBox2, 'LineWidth', 3)
set(gca, 'LineWidth', 3, 'FontSize', 28, 'FontWeight', 'bold')
